export interface GeneratedCard {
  ...
  export async function generateFlashcards(
    ...
    const prompt = `Generate ${count} educational flashcards about ${topic}. Each flashcard should have:
    1. A clear, focused question on the front
    2. A concise but informative answer on the back
    3. A helpful hint that guides without giving away the answer
    
    Format the response as a JSON array of objects with 'front', 'back', and 'hint' properties.`;
  
    const response = await makeGroqRequest(prompt);
    try {
      const cards = JSON.parse(response) as GeneratedCard[];
