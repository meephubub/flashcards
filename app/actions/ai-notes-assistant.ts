...
`;
  ...
  try {
    ...
    console.log(
      "[GROQ DEBUG] Sending request to Groq API:",
    ...
    if (!response.ok) {
      ...
      console.error(
        "[GROQ DEBUG] API request failed. Status:",
        response.status,
        response.statusText,
      );
    ...
    throw new Error(
      `Failed to process your question: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
...
`;
  ...
  try {
    ...
    console.log(
      "[GROQ DEBUG] Raw AI response for edits:",
    ...
    if (sanitizedContent !== aiResponseContent) {
      console.log(
        "[GROQ DEBUG] Original AI response content for edits:",
        aiResponseContent,
      );
      console.log(
        "[GROQ DEBUG] Sanitized AI response content for edits:",
        sanitizedContent,
      );
    }
    ...
    try {
      ...
      console.log(
        "[GROQ DEBUG] Parsed AI operations:",
      ...
      if (
        ...
        operations.some(
          (op) => !op.operation || typeof op.startIndex !== "number",
        )
      ) {
        console.error(
          "[GROQ DEBUG] Parsed operations (from sanitized content) do not match expected format. Sanitized content was:",
          sanitizedContent.substring(0, 500),
        );
        throw new Error(
          `AI response for edits, after sanitization, was not in the expected JSON array format or lacked required fields. Original content prefix: ${aiResponseContent ? aiResponseContent.substring(0, 200) : "N/A"}...`,
        );
      }
      return operations;
    } catch (parseError) {
      console.error(
        "[GROQ DEBUG] Failed to parse AI response for edits as JSON.",
      );
      console.error("[GROQ DEBUG] Original Content was:", aiResponseContent);
      if (sanitizedContent !== aiResponseContent && aiResponseContent) {
        console.error(
          "[GROQ DEBUG] Sanitized Content (which still failed to parse) was:",
          sanitizedContent,
        );
      }
      ...
      throw new Error(
        `AI response for edits was not valid JSON: ${parseError instanceof Error ? parseError.message : String(parseError)}. Original Response: ${aiResponseContent ? aiResponseContent.substring(0, 200) : "N/A"}...`,
      );
    }
  } catch (error) {
    ...
    if (
      ...
      error instanceof Error &&
      error.message.startsWith("AI response for edits was not valid JSON")
    ) {
      throw error; // Propagate JSON parsing errors as they are informative
    } else if (
      error instanceof Error &&
      error.message.startsWith("AI did not return any content")
    ) {
      throw error; // Propagate no content errors
    } else if (
      error instanceof Error &&
      error.message.startsWith(
        "AI response for edits was not in the expected JSON array format",
      )
    ) {
      throw error; // Propagate format errors
    }
    throw new Error(
      `Failed to generate edits: ${error instanceof Error ? error.message : "An unknown error occurred"}`,
    );
  }
}
