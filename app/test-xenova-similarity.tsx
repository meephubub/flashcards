...
export default function XenovaSimilarityTestPage() {
  ...
  return (
    <div style={{ maxWidth: 600, margin: "2rem auto", padding: 24, border: "1px solid #eee", borderRadius: 8 }}>
      ...
      <button onClick={handleCompare} disabled={loading || !sentenceA || !sentenceB}>
      ...
      {embeddingA && embeddingB && (
        <details style={{ marginTop: 16 }}>
          ...
          <div>
            <div><strong>Embedding A:</strong> [{Array.from(embeddingA).slice(0, 10).map(v => v.toFixed(4)).join(", ")} ...]</div>
            <div><strong>Embedding B:</strong> [{Array.from(embeddingB).slice(0, 10).map(v => v.toFixed(4)).join(", ")} ...]</div>
          </div>
        </details>
      )}
    </div>
