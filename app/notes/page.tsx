...
const parseInlineMarkdown = (text: string): React.ReactNode => {
  ...
  processedText = processedText.replace(
    /~~(.*?)~~/g,
    '<del class="text-neutral-500">$1</del>',
  );
...
const renderNoteContent = (
  ...
  gapStates?: Record<
    string,
    { value: string; similarity: number; isRevealed?: boolean }
  >,
  setGapStates?: React.Dispatch<
  ...
  dragDropStates?: Record<
    string,
    { answers: Record<number, string>; showAnswers: boolean }
  >,
  setDragDropStates?: React.Dispatch<
    React.SetStateAction<
      Record<string, { answers: Record<number, string>; showAnswers: boolean }>
    >
  >,
  noteCategory?: string,
  getCategoryColorClass?: (category: string) => {
    bg: string;
    accent: string;
    border: string;
  },
    ...
    if (tableRows.length > 0) {
      elements.push(
        <div key={`table-${elements.length}`} className="my-4 overflow-x-auto">
          <table
            className={`min-w-full border-collapse border ${isDark ? "border-neutral-700" : "border-gray-300"}`}
          >
            <thead>
              <tr>
              ...
              {tableRows.map((row, rowIndex) => (
                ...
                >
                  {row.map((cell: string, cellIndex: number) => (
                    <td
                      key={cellIndex}
                      className={`border px-3 py-2.5 text-sm whitespace-pre-wrap break-words ${
                        isDark
  ...
  const processInfoBox = () => {
    ...
    if (infoBoxContent.length > 0) {
      ...
      const colorClass =
        colorClasses[infoBoxColor as keyof typeof colorClasses] ||
        colorClasses.blue;
      const textColorClass =
        textColorClasses[infoBoxColor as keyof typeof textColorClasses] ||
        textColorClasses.blue;
      ...
      elements.push(
        <div
          key={`infobox-${elements.length}`}
          className={`my-4 p-4 rounded-lg border ${colorClass}`}
        >
      ...
      try {
        ...
        elements.push(
          <div
            ...
            className={`my-4 p-4 rounded-lg ${mathTextColor}`}
          >
            {mathString}
          </div>,
        );
  ...
  const processMcqBlock = () => {
    ...
    if (currentMcqQuestion && currentMcqOptions.length > 0) {
      ...
      const blockKeySlugPart = generateSlug(
        currentMcqQuestion.substring(0, 30),
      );
        ...
        >
          <div className="flex items-start mb-5">
            <div className="flex-shrink-0 mr-3">
              <HelpCircleIcon
                className={`${isDark ? "text-blue-400" : "text-blue-500"} flex-shrink-0`}
                size={20}
              />
            </div>
            <div
              className={`font-semibold ${isDark ? "text-lg text-neutral-100" : "text-xl text-gray-800"}`}
            >
          ...
          <div className={`space-y-3 ${isDark ? "pl-2" : "pl-3 pr-1"}`}>
            {shuffledOptions.map((option: McqOption, index: number) => {
              ...
              const showIncorrect =
                mcqState.showAnswers && isSelected && !option.isCorrect;
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-grow">
                    ...
                    {showCorrect && (
                      <div
                        className={`mt-1 animate-fade-in ${isDark ? "text-xs text-green-300" : "text-sm font-medium text-green-600"}`}
                      >
                        Correct answer!
                      </div>
                    )}
                    {showIncorrect && (
                      <div
                        className={`mt-1 animate-fade-in ${isDark ? "text-xs text-red-300" : "text-sm font-medium text-red-600"}`}
                      >
                        Incorrect - try again!
                      </div>
                    )}
          ...
          {mcqState.showAnswers && (
            <div
              className={`mt-5 ${isDark ? "text-sm text-neutral-400" : "text-sm font-medium text-gray-500 text-center"}`}
            >
  ...
  const processDragDropBlock = () => {
    if (dragDropLines.length > 0) {
      ...
      const blockState = dragDropStates?.[dragDropBlockKey] || {
        answers: {},
        showAnswers: false,
      };
          ...
          setUserAnswers={(fn) => {
            if (setDragDropStates) {
              setDragDropStates((prev) => {
                const currentBlockState = prev[dragDropBlockKey] || {
                  answers: {},
                  showAnswers: false,
                };
                const newAnswers =
                  typeof fn === "function" ? fn(currentBlockState.answers) : fn;
                return {
                  ...prev,
                  [dragDropBlockKey]: {
                    ...currentBlockState,
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("```tree") || line.startsWith("```mermaid")) {
      ...
      const { bg, accent, border } = getCategoryColorClass
      ...
      elements.push(
        ...
        >
          <div
            className={`flex items-center gap-3 px-8 pt-7 pb-3 border-b ${border} bg-gradient-to-r from-white/70 via-white/60 to-white/40 dark:from-neutral-900/80 dark:via-neutral-900/30 dark:to-neutral-900/60 backdrop-blur-md`}
          >
            <div
              className={`flex items-center justify-center w-11 h-11 rounded-xl ${accent} group-hover:${accent} transition`}
            >
              <svg
            ...
            <span className="font-semibold text-lg md:text-xl text-neutral-800 dark:text-neutral-100 tracking-tight drop-shadow-sm">
              Diagram
            </span>
            <span
              className={`ml-auto text-xs ${accent} font-mono uppercase tracking-wider`}
            >
              Mermaid
            </span>
          </div>
    ...
    if (headingMatch) {
      ...
      console.log(
        `[HEADING] Rendering heading: '${headingText}' with ID '${headingId}' and level ${level}`,
      );
    ...
    if (line.startsWith("?? ")) {
      ...
      console.log(
        `[MCQ PARSE] Set currentMcqQuestion: '${currentMcqQuestion}'`,
      );
    ...
    if (inMcqBlock) {
      console.log(
        `[MCQ PARSE] In MCQ block (question: '${currentMcqQuestion}'), checking line for option: '${line}'`,
      );
      ...
      if (optionMatch) {
        ...
        console.log(
          `[MCQ PARSE] Added option to '${currentMcqQuestion}':`,
          option,
          `Current options count: ${currentMcqOptions.length}`,
        );
        continue; // Consumed line, move to next line
      } else {
        ...
        console.log(
          `[MCQ PARSE] End of MCQ block detected (non-option line '${line}'). Processing MCQ for question: '${currentMcqQuestion}'`,
        );
        processMcqBlock();
    ...
    if (gapRegex.test(line)) {
      ...
      while ((match = gapRegex.exec(line)) !== null) {
        ...
        let colorClass = getGapColorClass(
          gapValue,
          answer,
          similarity,
          gapValue.length === 0,
            ...
            <Input
              ...
              onChange={(e) => {
                ...
                const isPartialMatch =
                  correctAnswer.startsWith(userInput) && userInput.length >= 3;
                ...
                if (distance <= 2 || isPartialMatch) {
                  ...
                  // Log for debugging
                  console.log(
                    `Autofilled gap ${gapKey} with answer: ${answer} (distance: ${distance}, partial: ${isPartialMatch})`,
                  );
                }
              }}
              ...
              onBlur={(e) => {
                ...
                if (distance <= 2 || isPartialMatch) {
                  ...
                  console.log(
                    `Autofilled gap ${gapKey} on blur with answer: ${answer} (distance: ${distance})`,
                  );
                }
              }}
              onKeyDown={(e) => {
                ...
                if (e.key === "Enter" || e.key === "Tab") {
                  ...
                  const allGapInputs = document.querySelectorAll(
                    'input[id^="gap-line-"]',
                  );
                  const currentIndex = Array.from(allGapInputs).findIndex(
                    (input) => (input as HTMLElement).id === gapKey,
                  );
                  const nextInput = allGapInputs[currentIndex + 1] as
                    | HTMLInputElement
                    | undefined;
                  if (nextInput) {
                    nextInput.focus();
      ...
      const filledGaps = Object.entries(gapStates || {}).filter(
        ([key, state]) =>
          key.startsWith(lineKey) &&
          (state.similarity > 0.6 || state.isRevealed),
      ).length;
            ...
            <div className="flex items-center space-x-3">
              ...
              <div className="w-24 h-2 bg-neutral-800 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${totalGaps > 0 ? (filledGaps / totalGaps) * 100 : 0}%`,
                  }}
                ></div>
              </div>
            </div>
          ...
          <div className="flex justify-end mt-4 pt-3 border-t border-neutral-800">
            <Button
              ...
              onClick={async () => {
                ...
                const gapKeys = Object.keys(gapStates || {}).filter((key) =>
                  key.startsWith(lineKey),
                );
                ...
                for (const key of gapKeys) {
                  ...
                  if (state && !state.isRevealed && state.value.length > 0) {
                    ...
                    const sim =
                      (await getSimilarity?.(state.value, answer)) || 0;
  ...
  if (inMcqBlock || (currentMcqQuestion && currentMcqOptions.length > 0)) {
    ...
    console.log(
      `[RENDER END] Processing any final MCQ block. Current Question: '${currentMcqQuestion}', Options: ${currentMcqOptions.length}`,
    );
  }
  ...
  ({
    ...
    return (
      <div className="flex items-center gap-3">
        <span className="min-w-[80px] font-medium text-neutral-200">
          {pair.left}
        </span>
        <div
...
const DragDropBlock = React.memo(
  ({
    question,
    pairs,
    options,
    ...
    const handleDragStart = useCallback(
      (e: React.DragEvent, option: string) => {
    ...
    return (
      <div
        className={`my-6 ${theme === "dark" ? "border border-neutral-700 bg-neutral-800/40 rounded-lg shadow-md p-5" : "bg-white rounded-lg shadow-md p-6 border border-gray-200"}`}
      >
        <div className="mb-4 font-semibold text-lg text-neutral-100">
          {question}
        </div>
...
const NoteCard = React.memo(
  function NoteCard({
    ...
    const categoryColorMap: {
      [key: string]: { bg: string; accent: string; border: string };
    } = {
      biology: isDark
        ? {
            bg: "bg-green-900/30",
    ...
    const getCategoryColorClass = (
      ...
      return (
        categoryColorMap[lowerCategory] ||
        (isDark
          ? {
              bg: "bg-neutral-800/30",
              accent: "bg-neutral-500/80",
              border: "border-neutral-700/60",
            }
          : {
              bg: "bg-gray-100",
              accent: "bg-gray-400/70",
    ...
    return (
      ...
      >
        ...
        <div
          className={`${categoryColors.bg} p-0.5 border-b ${categoryColors.border}`}
        >
          <div
            className={`h-1 w-16 ${categoryColors.accent} rounded-full mx-auto`}
          ></div>
        </div>
        ...
        <div className="relative overflow-hidden">
          <div className="p-6 md:p-8 pt-4">
            <div className="flex justify-between items-start mb-4 md:mb-6 pb-3 md:pb-4 border-b border-neutral-700/50">
              <h3
                className={`text-2xl md:text-3xl font-bold mr-2 ${isDark ? "text-neutral-50" : "text-gray-800"}`}
              >
                {note.title}
              </h3>
            ...
            <div
              className={`prose-custom max-w-none text-sm md:text-base ${isDark ? "text-neutral-300" : "text-neutral-700"}`}
            >
              {inlineEditingNoteId === note.id ? (
                <div className="relative">
                  <Textarea
                    ...
                    className={`min-h-[300px] w-full rounded-lg ${
                      isDark
            ...
            <div
              className={`flex items-center justify-between text-xs mt-6 md:mt-8 pt-3 md:pt-4 border-t ${isDark ? "border-neutral-800" : "border-gray-200"}`}
            >
              <div
                className={`flex items-center ${isDark ? "text-neutral-500" : "text-gray-500"}`}
              >
                <span
                  className={`mr-2 px-2 py-0.5 rounded-full text-[10px] font-medium uppercase tracking-wide ${categoryColors.bg} ${categoryColors.border} border
  ...
  (prevProps, nextProps) => {
    ...
    const isThisNotesFocusStatusChanged =
      (prevProps.note.id === prevProps.focusedNoteId) !==
      (nextProps.note.id === nextProps.focusedNoteId);
    const isThisNoteBeingEdited =
      (prevProps.note.id === prevProps.inlineEditingNoteId) !==
      (nextProps.note.id === nextProps.inlineEditingNoteId);
...
export default function NotesPage() {
  ...
  const [currentNoteHeadings, setCurrentNoteHeadings] = useState<
  ...
  const [currentQuizNoteTitle, setCurrentQuizNoteTitle] = useState<
    string | null
  >(null);
  const [userAnswers, setUserAnswers] = useState<Record<number, string>>({});
  const [showQuizResults, setShowQuizResults] = useState(false);
  const [isEditNoteDialogOpen, setIsEditNoteDialogOpen] =
    useState<boolean>(false);
  ...
  const [isImageSearchDialogOpen, setIsImageSearchDialogOpen] =
    useState<boolean>(false);
  ...
  const [mcqStates, setMcqStates] = useState<
    Record<string, { selectedIndex?: number; showAnswers: boolean }>
  >({});

  // Fill-in-the-gap state: { [gapKey]: { value, similarity, isRevealed } }
  const [gapStates, setGapStates] = useState<
    Record<string, { value: string; similarity: number; isRevealed?: boolean }>
  >({});

  // Drag and Drop state
  const [dragDropStates, setDragDropStates] = useState<
    Record<string, { answers: Record<number, string>; showAnswers: boolean }>
  >({});

  // State for inline editing
  const [inlineEditingNoteId, setInlineEditingNoteId] = useState<string | null>(
    null,
  );

  const [isGeneratingImages, setIsGeneratingImages] = useState(false);
  const [selectedImageModel, setSelectedImageModel] =
    useState<ImageModel>("flux");
  ...
  const activeNoteRef = useRef<HTMLDivElement>(
    null,
  ...
  useEffect(() => {
    if (focusedNoteId && displayedNotes.length > 0) {
      const focusedNoteIndex = displayedNotes.findIndex(
  ...
  const handleMcqOptionClick = useCallback(
    (blockId: number | string, optionIndex: number, isCorrect: boolean) => {
      const blockKey =
        typeof blockId === "number" ? blockId.toString() : blockId;
      setMcqStates((prev) => ({
        ...prev,
        [blockKey]: {
          ...prev[blockKey],
  ...
  const handleSaveInlineEdit = useCallback(
    async (noteId: string, content: string) => {
      if (!noteId || !content) {
        setErrorMessage(
          "Cannot save empty content or no note selected for inline edit.",
        );
        return;
      }
      ...
      try {
        ...
        if (error) {
          ...
          const updateNotesState = (prevNotes: Note[]) =>
            prevNotes.map((note) =>
              note.id === noteId
                ? {
                    ...note,
  ...
  const handleAddNote = useCallback(
    async (
      note: Omit<
        Note,
        "id" | "created_at" | "updated_at" | "user_id" | "flashcards"
      >,
    ) => {
      setIsLoading(true);
      ...
      try {
        ...
        if (data) {
          if (
            ...
            data.category === selectedSidebarCategory
          ) {
      ...
      const fetchImages = async () => {
        ...
        try {
          const response = await fetch(
            `/api/image-search?query=${encodeURIComponent(imageSearchQuery)}`,
          );
          if (!response.ok) {
            const errorData = await response.json().catch(() => ({
              error: `Failed to fetch images: ${response.status}`,
            }));
            throw new Error(
          ...
          setImageSearchError(
            err.message ||
              "An unknown error occurred while searching for images.",
          );
        } finally {
  ...
  const handleImageSearchTrigger = (
    ...
    if (match && match[1]) {
      ...
      if (contextType === "newNote") {
        ...
        setEditingNote((prev) =>
          prev ? { ...prev, content: newTextValue } : null,
        );
      }
  ...
  useEffect(() => {
    ...
    if (selectedSidebarCategory !== "all") {
      notesToDisplay = allNotes.filter(
        (note) => note.category === selectedSidebarCategory,
      );
    }
      ...
      if (e.ctrlKey) {
        if (e.key === "k") {
          ...
          if (selection && selection.toString().trim()) {
            ...
            const noteElement =
              parentElementForClosest?.closest('[id^="note-"]');
            ...
            if (noteElement) {
              ...
              if (note) {
                ...
                if (isHighlighted) {
                  ...
                  if (firstIndex !== -1) {
                    updatedContent =
                      ...
                      noteContent.substring(
                        firstIndex + highlightedPattern.length,
                      );
                  }
                } else {
                  ...
                  if (plainTextIndex !== -1) {
                    updatedContent =
                      ...
                      highlightedVersion +
                      noteContent.substring(
                        plainTextIndex + selectedText.length,
                      );
                  } else {
                    console.warn(
                      "Text to highlight not found in note content:",
                      selectedText,
                    );
                    return;
                  }
                ...
                if (
                  ...
                  console.warn(
                    "Highlight toggle resulted in no change to content.",
                  );
                  return;
                }
          ...
          if (selection && selection.rangeCount > 0) {
            ...
            const noteContentElement = parentElement?.closest<HTMLElement>(
              '[id^="note-content-"]',
            );
            if (noteContentElement) {
              ...
              const noteElement =
                parentElement?.closest<HTMLElement>('[id^="note-"]');
              if (noteElement) {
          ...
          if (!targetNoteId && document.activeElement) {
            ...
            if (noteContentElement) {
              ...
              const noteElement =
                document.activeElement.closest<HTMLElement>('[id^="note-"]');
              if (noteElement) {
                targetNoteId = noteElement.id.replace("note-", "");
              }
            }
  ...
  const handleNoteSelectInSidebar = useCallback(
    (noteId: string) => {
  ...
  const handleSubheadingClick = (subheading: string) => {
    const targetNote = allNotes.find((note) => {
      ...
      return lines.some(
        (line) =>
          line.match(/^##\s+(.*)/) &&
          line.match(/^##\s+(.*)/)![1].trim() === subheading,
      );
    });
    if (targetNote) {
      if (
        selectedSidebarCategory !== "all" &&
        targetNote.category !== selectedSidebarCategory
      ) {
  ...
  const handleGenerateNote = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.details || "Failed to generate note from API.",
        );
      }
  ...
  const handleGenerateMcqs = async () => {
    ...
    const noteToGenerateFrom = allNotes.find(
      (note) => note.id === focusedNoteId,
    );
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.error || "Failed to generate quiz questions from API.",
        );
      }
      ...
      if (data.questions && data.questions.length > 0) {
        ...
        setQuizError(
          "No quiz questions were generated for this note. The content might be too short or not suitable.",
        );
      }
    } catch (error: any) {
      ...
      setQuizError(
        error.message ||
          "An unexpected error occurred while generating quiz questions.",
      );
    } finally {
    ...
    if (focusedNoteId) {
      ...
      if (noteToEdit) {
        ...
        try {
          ...
          if (updateError) {
            console.error(
              "Error updating note after image selection:",
              updateError,
            );
          ...
          console.error(
            "Exception during note update after image selection:",
            error,
          );
  ...
  const handleGenerateFlashcards = async () => {
    ...
    try {
      ...
      setActiveNote(
        updatedNotes.find((note) => note.id === noteForFlashcards.id) || null,
      );
    } catch (error) {
  ...
  const SyntaxDocs = ({ isDark }: { isDark: boolean }) => (
    <div className="flex flex-col md:flex-row gap-0 md:gap-8 w-full">
      ...
      <aside
        className={`md:w-80 w-full md:rounded-l-2xl rounded-t-2xl md:rounded-t-none p-6 md:p-8 flex-shrink-0 border-r ${isDark ? "bg-neutral-950/95 border-neutral-800" : "bg-white/95 border-neutral-200"} shadow-none`}
      >
        <h2 className="flex items-center gap-2 text-2xl font-extrabold mb-6 tracking-tight text-neutral-900 dark:text-neutral-100">
          <SparklesIcon
            className={`inline-block h-6 w-6 ${isDark ? "text-neutral-200" : "text-neutral-800"}`}
          />{" "}
          Syntax Guide
        </h2>
        <ul className="mb-8 space-y-1 text-sm text-neutral-700 dark:text-neutral-300">
          <li>
            <b>Headings:</b> <code># Heading 1</code>, <code>## Heading 2</code>
            , <code>### Heading 3</code>
          </li>
          <li>
          ...
          <li>
            <b>Highlight:</b> <code>==highlight==</code>
          </li>
          <li>
            <b>Strikethrough:</b> <code>~~strikethrough~~</code>
          </li>
          <li>
            <b>Links:</b> <code>[text](url)</code>
          </li>
          <li>
            <b>Images:</b> <code>![alt](url)</code> or{" "}
            <code>![alt|maxheight=300](url)</code>
          </li>
          <li>
            <b>LaTeX Math:</b> <code>$inline$</code> or <code>$$block$$</code>
          </li>
          <li>
            <b>Horizontal Rule:</b> <code>---</code>
          </li>
          <li>
            <b>Info Boxes:</b> <code>::blue</code> ... <code>::</code> (also
            purple, green, amber, rose)
          </li>
          <li>
            <b>Tables:</b> <code>| Col1 | Col2 |</code>
          </li>
          <li>
            <b>Lists:</b> <code>- item</code> or <code>* item</code>
          </li>
          <li>
            <b>MCQ Block:</b> <code>?? Question</code> then{" "}
            <code>[x] Correct</code> <code>[ ] Incorrect</code>
          </li>
          <li>
            <b>Fill-in-the-gap:</b> <code>[gap:answer]</code>
          </li>
          <li>
            <b>Drag & Drop:</b> <code>::dragdrop</code> ... <code>::</code> (see
            docs)
          </li>
          <li>
            <b>Diagrams:</b> <code>{"```mermaid"}</code> ...{" "}
            <code>{"```"}</code>
          </li>
        </ul>
        <h3 className="flex items-center gap-2 text-base font-semibold mb-3 text-neutral-900 dark:text-neutral-100">
          <Keyboard
            className={`inline-block h-4 w-4 ${isDark ? "text-neutral-400" : "text-neutral-700"}`}
          />{" "}
          Shortcuts
        </h3>
        <ul className="mb-2 space-y-1 text-sm text-neutral-700 dark:text-neutral-300">
          <li>
            <b>Ctrl+K</b>: AI chat about selected text
          </li>
          <li>
            <b>Ctrl+B</b>: Highlight/unhighlight selected text
          </li>
          <li>
            <b>Ctrl+E</b>: Edit note inline or toggle AI assistant
          </li>
          <li>
            <b>Ctrl+S</b>: Toggle sidebar
          </li>
          <li>
            <b>Ctrl+H</b>: Show this help
          </li>
        </ul>
      </aside>
      {/* Main: Examples and AI Format */}
      <main
        className={`flex-1 min-w-0 p-6 md:p-10 flex flex-col justify-between bg-neutral-50 dark:bg-neutral-950 rounded-b-2xl md:rounded-r-2xl md:rounded-bl-none`}
      >
        <div>
          <h3 className="flex items-center gap-2 text-lg font-bold mb-4 text-neutral-900 dark:text-neutral-100">
            <InfoIcon
              className={`inline-block h-5 w-5 ${isDark ? "text-neutral-200" : "text-neutral-800"}`}
            />{" "}
            Examples
          </h3>
          <div
...
`;
      const aiNote = await formatNoteWithGroq(
        note.content,
    ...
    >
      ...
      <div className="md:hidden fixed top-4 left-4 z-50">
        <Sheet>
          ...
          <SheetContent
            side="left"
            className="p-0 border-r border-neutral-800 w-4/5 max-w-[300px] bg-neutral-950 rounded-r-xl"
          >
            <NotesSidebar
              notes={allNotes}
      ...
      >
        ...
        <div
          ...
          className="flex-1 overflow-y-auto p-4 pt-14 md:pt-4 md:p-6 lg:p-8 pb-24 scroll-smooth no-scrollbar"
        >
          {/* Search results count - only shown when searching */}
          {searchQuery && (
            <div className="text-xs text-neutral-400 mb-6 ml-1">
              Found {displayedNotes.length}{" "}
              {displayedNotes.length === 1 ? "result" : "results"} for "
              {searchQuery}"
            </div>
          )}
          <div
          ...
          >
            ...
            {!isLoading &&
              ...
              !searchQuery && (
                <div className="text-center text-neutral-400 py-10 text-lg">
                  ...
                  >
                    <PlusCircleIcon className="mr-2 h-5 w-5" /> Add Your First
                    Note
                  </Button>
                </div>
              )}
            {!isLoading &&
              ...
              !searchQuery && (
                <p className="text-center text-neutral-400 py-10 text-lg">
                  No notes in "
                  {selectedSidebarCategory.charAt(0).toUpperCase() +
                    selectedSidebarCategory.slice(1)}
                  ".
                </p>
              )}
            ...
            {!isLoading &&
              ...
              !searchQuery && (
                <p className="text-center text-neutral-400 py-10 text-lg">
                  Note not found or does not match category.
                </p>
              )}
        ...
        >
          ...
          <div
            className="absolute inset-0 z-[-1] pointer-events-none rounded-3xl"
            style={{
              background:
                theme === "dark"
          ...
          {isExamActive && examStats ? (
            <div className="w-full flex items-center justify-between px-2 text-sm">
              <div className="flex items-center space-x-4 font-mono">
                <div>
                  ...
                  <span>
                    {examStats.currentQuestion}/{examStats.totalQuestions}
                  </span>
                </div>
                <div>
            ...
            <>
              <div className="flex items-center space-x-1.5">
                ...
                <span
                  className={`hidden md:inline-flex items-center text-xs rounded-xl px-2 py-1 transition-all duration-200 ${theme === "dark" ? "text-neutral-400 border border-white/[0.06] bg-white/[0.02]" : "text-gray-500 border border-black/[0.04] bg-black/[0.01]"}`}
                >
                  <SparklesIcon className="h-2.5 w-2.5 mr-1" />{" "}
                  <kbd className="font-mono text-[9px] font-medium">Ctrl+K</kbd>
                </span>
              </div>
                ...
                <ScrollArea className="w-full whitespace-nowrap">
                  <div className="flex h-8 items-center space-x-1">
                    {focusedNoteId && currentNoteHeadings.length > 0 ? (
                        ...
                        <Button
                          ...
                          onClick={() => {
                            const headingEl = document.getElementById(
                              `heading-${generateSlug(heading.text)}`,
                            );
                            if (headingEl)
                          ...
                          className={`transition-all rounded-xl px-3 py-1.5 text-xs font-medium ${theme === "dark" ? "text-neutral-300 hover:text-white hover:bg-white/[0.06] data-[state=active]:bg-white/[0.1] data-[state=active]:text-white" : "text-gray-600 hover:text-gray-900 hover:bg-black/[0.04] data-[state=active]:bg-black/[0.08] data-[state=active]:text-gray-900"} ${
                            heading.level === 1
                              ? "font-semibold"
                              : heading.level === 2
                                ? "pl-2"
                      ...
                      <p
                        className={`text-xs px-2 ${theme === "dark" ? "text-neutral-500" : "text-gray-500"}`}
                      >
                        No H2 subheadings (##)
                      </p>
                    )}
              ...
              <Button
                ...
                className={`flex-shrink-0 border rounded-2xl px-4 py-2 transition-all duration-300 text-xs font-medium hover:scale-105 ${
                  theme === "dark"
                    ? "text-neutral-200 bg-white/[0.03] border-white/[0.08]" +
                      (!focusedNoteId
                        ? " opacity-50"
                    ...
                    : "bg-black/[0.02] border-black/[0.06]" +
                      (!focusedNoteId
                        ...
                        : " text-gray-700 hover:text-gray-900 hover:bg-black/[0.06] hover:border-black/[0.12]")
                }`}
              >
                {isGeneratingQuiz ? (
                  <>
          ...
          <div className="p-6 sm:p-8">
            ...
            <div className="space-y-4">
              <div className="p-3 bg-neutral-800/50 border border-neutral-700 rounded-md">
                ...
                <p className="text-neutral-200 font-mono text-sm whitespace-pre-wrap">
                  {selectedText}
                </p>
              </div>
              ...
              <div className="flex space-x-2">
                ...
                <Button
                  ...
                  disabled={
                    ...
                    (!aiPrompt.trim() && !selectedText.trim())
                  }
