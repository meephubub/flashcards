...
export function AIAssistantSidebar({ 
  ...
  return (
    <div 
      className={`fixed top-0 right-0 h-full bg-background border-l border-border shadow-xl z-40 flex flex-col overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'}`}
      style={{
    ...
    >
      <div className="flex items-center justify-between p-4 border-b border-border">
        <div className="flex items-center space-x-2">
          {isEditMode ? (
            <PencilLine size={18} className="text-blue-500 dark:text-blue-400" />
          ) : (
        ...
        <div className="flex items-center space-x-2">
          <Button
            ...
            size="sm"
            onClick={toggleMode}
            className={`text-xs p-1.5 rounded-md ${isEditMode ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400' : 'text-muted-foreground hover:text-foreground'}`}
          >
                  ...
                  <div 
                    className={`max-w-[300px] p-3 rounded-lg ${
                      message.role === 'user'
                        ...
                        : (message.isEditing ? "bg-card" : "bg-muted text-foreground")
                    }`}
                  >
                    {message.isLoading ? (
                      <div className="flex items-center space-x-2">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="bg-card p-3 rounded-md text-foreground border border-border">
                          <p className="text-sm font-medium mb-2 text-foreground">{message.content}</p>
                          <div className="text-xs max-h-60 overflow-y-auto border border-border p-2 rounded bg-background">
                            <DiffView
                          ...
                          <div className="flex justify-end space-x-2 pt-2">
                            <Button
                              size="sm"
                              variant="outline"
                              className="bg-background border-border hover:bg-accent text-xs px-2 py-1 h-auto"
                            ...
                            <Button
                              size="sm"
                              variant="outline"
                              className="bg-card text-destructive border-border hover:bg-accent text-xs px-2 py-1 h-auto"
                              onClick={() => {
                                setMessages(prev =>
                                  prev.map(msg =>
                            ...
                            <Button
                              size="sm"
                              className="bg-card text-green-600 dark:text-green-400 border-border hover:bg-accent text-xs px-2 py-1 h-auto"
      ...
      <div className="p-4 border-t border-border">
        ...
        >
          <Input
            ...
            className="flex-1 bg-background border-border focus:border-ring text-foreground"
            disabled={isLoading}
          />
          <Button
            type="submit"
            ...
            className={`bg-primary hover:bg-primary/90 text-primary-foreground ${!input.trim() ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            <SendIcon size={16} />
          </Button>
        </form>
          ...
          <div className="flex flex-col items-center space-y-2">
            {lastAppliedEdit && (
              <Button
                ...
                className="text-xs text-muted-foreground hover:text-foreground border-border hover:bg-accent px-2 py-1 h-auto w-full max-w-xs"
              >
                <Undo2 size={14} className="mr-1" /> Revert Last Applied Edit
              </Button>
            )}
            <p className="text-muted-foreground text-xs">
              Press <kbd className="px-1 py-0.5 bg-muted rounded text-muted-foreground">Ctrl+E</kbd> to toggle mode
            </p>
          </div>
        </div>
      </div>
