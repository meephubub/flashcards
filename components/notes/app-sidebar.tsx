...
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener(
      ...
      openImport as EventListener,
    );
    window.addEventListener(
      "open-generate-flashcards",
      openGenerate as EventListener,
    );
    window.addEventListener("open-merge-decks", openMerge as EventListener);
    window.addEventListener(
      "open-create-model",
      openCreateModel as EventListener,
    );
    return () => {
      window.removeEventListener(
        "open-create-deck",
        openCreate as EventListener,
      );
      window.removeEventListener(
        "open-import-markdown",
        openImport as EventListener,
  ...
  return (
    <>
                ...
                <div className="mt-3">
                  ...
                  <ul className="mt-1 max-h-28 overflow-auto divide-y divide-border rounded-md">
                    {upcomingTasks.length === 0 ? (
                      <li className="py-2 px-2 text-xs text-muted-foreground">
                        No upcoming tasks
                      </li>
                    ) : (
                      upcomingTasks.map((t) => (
      ...
      />
      <MergeDecksDialog
        isOpen={isMergeDecksOpen}
        onOpenChange={setIsMergeDecksOpen}
      />
      <CreateModelDialog
